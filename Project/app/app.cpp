...
void Application::PrintUnnecessaryDirectory(SongId3& song_id3) {
  ...
  if (!FileSystem::GetDirectories(album_absolute_path, unnecessary_folders)) {
    is_valid = false;
  ...
  for (uint32_t i = 0; i < unnecessary_folders.size(); i++) {
    is_valid = false;
    if (!FileSystem::GetDirectoryName(unnecessary_folders.at(i), unnecessary_folder_name)) {
...
void Application::PrintUnnecessaryFiles() {
  ...
  for (uint32_t i = 0; i < unnecessary_files.size(); i++) {
    ...
    if (!FileSystem::GetFileName(unnecessary_files.at(i), unnecessary_file_name)) {
      Print("\t\tError: unnecessary file found: \"" + unnecessary_files.at(i) + "\"", display_errors);
      continue;
    }

    if (!FileSystem::GetFileExtension(unnecessary_files.at(i), extension)) {
      Print("\t\tError: unnecessary file found: \"" + unnecessary_files.at(i) + "\"", display_errors);
      continue;
    }
...
bool Application::SortAlbumFiles(SongId3& song_id3) {
  ...
  if (!FileSystem::GetFiles(album_absolute_path, files)) {
    is_valid = false;
  ...
  for (uint32_t i = 0; i < files.size(); i++) {
    if (!FileSystem::GetFileName(files.at(i), file_name)) {
      is_valid = false;
    ...
    if (!FileSystem::GetFileExtension(files.at(i), file_extension)) {
      is_valid = false;
...
void Application::GetAlbumTitle(SongId3& song_id3) {
  if (!FileSystem::GetDirectoryName(album_absolute_path, song_id3.album_title)) {
    is_valid = false;
...
bool Application::GetAlbumsPath() {
  if (!FileSystem::GetDirectories(artist_absolute_path, album_absolute_paths)) {
    is_valid = false;
...
void Application::FindUnnecessaryFiles() {
  ...
  if (!FileSystem::GetFiles(artist_absolute_path, unnecessary_files)) {
    is_valid = false;
...
void Application::GetArtistName() {
  if (!FileSystem::GetDirectoryName(artist_absolute_path, artist_name)) {
    is_valid = false;
  ...
  while (1) {
    ...
    if (user_selection.size() > 0) {
      ...
      case '1':
        ...
        if (!FileSystem::Exist(artist_absolute_path)) {
          Print("Artist path does not exist: \"" + artist_absolute_path + "\"");
          artist_absolute_path = "";
          break;
        }

        if (!FileSystem::IsDirectory(artist_absolute_path)) {
